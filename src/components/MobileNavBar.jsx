    ...
    <nav className={`${showNav ? "show__nav" : ""} mobile__nav `}>
      <ul className={`links ${theme === "dark" ? "bg-slate-800" : "bg-white"}`}>
        {topics.map(({ topic, icon }) => {
          return (
            <li key={topic} className={`link capitalize `}>
              <Link
                onClick={() => setShowNav(false)}
                to={`/articles?topic=${topic}  `}
                className={`flex gap-4 justify-center items-center flex-row`}
              >
                {topic} {icon}
              </Link>
            </li>
      ...
      <ul className="socials flex flex-col">
        <p
          className={` ${
            theme === "dark" ? "bg-white text-red-500" : "bg-red-500 text-white"
          } bg-red font-bold text-xl rounded p-2 `}
        >
          Hello {user}
        </p>
